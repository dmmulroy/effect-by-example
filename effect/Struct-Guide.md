...
const productSummary = pipe(
  product,
  Struct.pick("id", "name", "price")
)
// Type: { readonly id: string; readonly name: string; readonly price: number }

// Omit sensitive properties
const publicProduct = pipe(
  product,
  Struct.omit("inStock", "description")
)
...
// Get property value
const databaseConfig = Struct.get("database")(config)
...
```typescript
import { Struct, pipe, Effect } from "effect"
...
const createPublicProfile = (profile: UserProfile) =>
  pipe(
    profile,
    Struct.pick("id", "username", "firstName", "lastName", "avatar", "isVerified"),
    Struct.evolve({
      firstName: (name) => name.charAt(0).toUpperCase() + name.slice(1),
      lastName: (name) => name.charAt(0).toUpperCase() + name.slice(1)
    })
...
const createAdminView = (profile: UserProfile) =>
  pipe(
    profile,
    Struct.omit("preferences"), // Remove personal preferences
    Struct.evolve({
      email: (email) => email.toLowerCase(),
...
const createUserSettings = (profile: UserProfile) =>
  pipe(
    profile,
    Struct.pick("id", "email", "preferences"),
    Struct.evolve({
      email: (email) => email.toLowerCase()
    })
  )

// Usage in an Effect workflow
...
// Mock service
const getUserProfile = (userId: string): Effect.Effect<UserProfile, Error> =>
...
```typescript
import { Struct, pipe, Effect, Array as Arr } from "effect"
...
// Mobile API - compact response
const createMobileResponse = (product: ProductResponse) =>
  pipe(
    product,
    Struct.pick("id", "name", "price", "currency"),
    Struct.evolve({
...
const createDashboardResponse = (product: ProductResponse) =>
  pipe(
    product,
    Struct.omit("metadata"), // Remove internal metadata
    Struct.evolve({
      name: (name) => name.split(' ')
        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' '),
...
const getDashboardProducts = () =>
  Effect.gen(function* () {
    ...
    return pipe(
      allProducts,
      Arr.map(createDashboardResponse)
    )
  })
...
const getInventoryReport = () =>
  Effect.gen(function* () {
    ...
    return pipe(
      allProducts,
      Arr.map(createInventoryResponse)
    )
  })
...
```typescript
import { Struct, pipe, Effect, Option } from "effect"
...
// Complex order processing with validation and normalization
...
```typescript
import { Struct, pipe, Option } from "effect"
...
// Conditional transformations based on subscription tier
...
```typescript
import { Struct, String, Number, Boolean, Array as Arr, pipe, Effect } from "effect"
...
```typescript
import { Struct, String, Number, Order, Array as Arr, pipe } from "effect"
...
// Sort employees using struct order
const sortedEmployees = pipe(
  employees,
  Arr.sort(EmployeeOrder)
)
```
...
const sortTasksForDashboard = (tasks: Task[]) =>
  pipe(
    tasks,
    Arr.filter(task => task.status !== "done"), // Only active tasks
    Arr.sort(Order.combine(
...
// Create different views with sorting
const createTaskViews = (tasks: Task[]) => ({
  priorityView: pipe(
    tasks,
    Arr.sort(Order.reverse(PriorityOrder))
  ),
  dueDateView: pipe(
    tasks,
    Arr.sort(Order.mapInput(String.Order, (task: Task) => task.dueDate))
  ),
  assigneeView: pipe(
    tasks,
    Arr.sort(Order.mapInput(String.Order, (task: Task) => task.assigneeId))
  ),
  workloadView: pipe(
    tasks,
    Arr.sort(Order.reverse(Order.mapInput(Number.Order, (task: Task) => task.estimatedHours)))
  )
})
```
...
const normalizeProduct = createDataNormalizer<ProductInput>({
  name: (name) => pipe(name, transformers.sanitizeString, transformers.capitalizeString),
  ...
  tags: (tags) => pipe(tags, transformers.uniqueStrings, transformers.sortStrings),
...
// Type-safe mappers using Struct operations
const mapDatabaseToApi = (dbUser: DatabaseUser): ApiUser => {
  const picked = pipe(
    dbUser,
    Struct.pick("user_id", "first_name", "last_name", "email_address", "created_timestamp", "is_active")
  )
...
const mapApiToDisplay = (apiUser: ApiUser): DisplayUser =>
  pipe(
    apiUser,
    Struct.pick("fullName", "email", "joinDate", "status"),
    Struct.evolve({
      fullName: (name) => name,
      email: (email) => email,
...
```typescript
import { Struct, pipe, Effect, Either, Array as Arr } from "effect"
...
```typescript
import { Struct, pipe, Effect, Schema } from "effect"
...
const processApiData = (rawData: RawApiData) =>
  Effect.gen(function* () {
    ...
    // Apply struct transformations after validation
...
const createDataViews = (processedData: {
  ...
  activeUserProducts: processedData.users
    ...
    .map(user => pipe(user, Struct.pick("name", "email")))
})
```
...
// Transform API data to UI props
const transformUserForCard = (apiUser: ApiUser): UserCardProps =>
  pipe(
    apiUser,
    Struct.pick("user_id", "first_name", "last_name", "email_address", "profile_image_url", 
...
```typescript
import { Struct, pipe, Effect, Equal, Array as Arr } from "effect"
...
describe("Struct Operations", () => {
  describe("pick", () => {
    it("should pick specified properties", () => {
      ...
      const picked = pipe(user, Struct.pick("id", "name"))
    ...
    it("should work with empty selection", () => {
      const user = createTestUser()
      const picked = pipe(user, Struct.pick())
  ...
  describe("omit", () => {
    it("should omit specified properties", () => {
      const user = createTestUser()
      const omitted = pipe(user, Struct.omit("email", "age"))
    ...
    it("should preserve all other properties", () => {
      ...
      const omitted = pipe(user, Struct.omit("email"))
  ...
  describe("property-based tests", () => {
    it("pick and omit should be complementary", () => {
      ...
      const picked = pipe(user, Struct.pick(...someKeys))
      const omitted = pipe(user, Struct.omit(...someKeys))
...
describe("Real-world integration", () => {
  it("should handle user profile transformation pipeline", async () => {
    ...
    const transformUser = pipe(
      rawUserData,
      Struct.pick("user_id", "first_name", "last_name", "email_address", "account_status"),
      Struct.evolve({
